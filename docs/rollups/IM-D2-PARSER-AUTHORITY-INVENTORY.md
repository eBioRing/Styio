...
| Resource-method receiver property route | Resource method/property definition bodies parse receiver postfix property access such as `@file.path` as a scoped `ResourceReceiverAST` plus `AttrAST`, while `@file.path` outside a resource-family definition remains fail-closed instead of becoming a constructor/property shortcut | `StyioSecurityNightlyParserStmt.ParsesResourceReceiverPropertyAccessInsideDefinitions`, `StyioSecurityNightlyParserStmt.RejectsResourceReceiverPropertyOutsideDefinition` |
