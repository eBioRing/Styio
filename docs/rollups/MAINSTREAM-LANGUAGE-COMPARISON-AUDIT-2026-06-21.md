...
| Type system growth path | Constraint solving is a mature compiler baseline, but Styio must define how resource capabilities, explicit `? | T` absence, matrix/container families, and future generics interact. D02 now excludes ordinary value fallback/coalescing. | Decide which remaining constraints are type equality, assignability, callable/indexable checks, resource capabilities, effect families, or explicit Optional constraints; do not introduce fallback constraints. |
