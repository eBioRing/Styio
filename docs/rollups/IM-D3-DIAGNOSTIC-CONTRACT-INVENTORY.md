...
| Sema, type, and lowering baseline | `STYIO_SEMA_IMMUTABLE_BINDING`, `STYIO_TYPE_ERROR`, `STYIO_TYPE_UNSUPPORTED_TUPLE_RETURN`, `STYIO_TYPE_STREAM_HASH_TAG_ROUTE_UNSUPPORTED`, `STYIO_TYPE_STREAM_ZIP_UNSUPPORTED_SOURCE`, `STYIO_LOWER_UNSUPPORTED_AST` |
...
IM-D3's public diagnostic baseline is implemented: every current public JSON/JSONL, IDE, and LSP diagnostic path now carries a machine-readable code and phase, and the main syntax-check, compile-plan, runtime, type, IDE, and LSP surfaces have focused tests. Current D3-B2 family refinements include immutable/final binding mutation failures reporting `STYIO_SEMA_IMMUTABLE_BINDING`, tuple function return annotations reporting `STYIO_TYPE_UNSUPPORTED_TUPLE_RETURN`, undefined hash-tag stream routes reporting `STYIO_TYPE_STREAM_HASH_TAG_ROUTE_UNSUPPORTED`, and stream zip unsupported-source failures reporting `STYIO_TYPE_STREAM_ZIP_UNSUPPORTED_SOURCE` instead of the broad `STYIO_TYPE_ERROR`. Remaining work is not an IM-D3 blocker; it is family refinement that replaces broad fallback codes such as `STYIO_TYPE_ERROR`, `STYIO_RUNTIME_ERROR`, and `STYIO_NATIVE_INTEROP_ERROR` with narrower diagnostics when a later feature checkpoint owns those error families.
