...
| Resource object selectors `@price[-1]`, `@price[-3..]`, `@price[...]` | **Implemented for v2 resource reads and bounded selector iterators across scalar/string/char/bool/f64/i64 plus list-valued and dict-valued resources; `@price[-3..]` / `@price[...]` materialize iterable snapshots, scalar latest selector iterators stay rejected, bounded matrix selectors stay rejected, and retired state-history probes stay rejected** |
| Explicit selector/container copy `snapshot << @price[-n..]` / `snapshot << @price[...]` / `copy << list_or_dict_or_matrix` | **Implemented for bounded `i64`, `f64`, `bool`, `char`, `string`, `list`, and `dict` selector snapshots plus materialized list/dict/matrix handle deep clones; scalar latest reads, `copy <- list_or_dict_or_matrix`, and broader file/topology `<<` clone/copy remain staged or rejected** |
