...
| Statement-preface return | `@file::answer = () => { >_("inside") <\| 42 }` | Value-returning method bodies may run accepted statement-only prefaces before a final returned expression. Scalar local `=` or `:=` prefaces such as `x = 41; <\| x + 1`, plus local list/dict `=` or `:=` prefaces such as `xs = [40, 2]; <\| xs` or `xs = [40, 2]; <\| xs[0] + xs[1]`, are scoped to the inlined method body when the final return is scalar/string or the local list/dict container value; local matrix/resource bindings and capture-dependent bodies remain fail-closed. |
