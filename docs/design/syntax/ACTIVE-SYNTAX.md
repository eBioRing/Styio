...
| Resource effect fallback | `?\| resource_operation \| fallback` | `?\| resource_operation` settles in place and raises immediately on failure; fallback participates in type inference. Statement resource operations include resource method calls such as `?\| @file("log.txt").close() \| fallback`; value-producing forms include stdin pulls with explicit target types, such as `result: f64 = ?\| (<- @stdin) \| fallback`, materialized container bounds reads such as `value = ?\| items[i] \| fallback`, `slice = ?\| items[0..] \| fallback`, `value = ?\| table[key] \| fallback`, `value = ?\| matrix[row][col] \| fallback`, or `row = ?\| matrix[row] \| fallback`, and single-return resource method calls such as `value = ?\| log.answer() \| fallback`. Bare `\| fallback` is not resource fallback. |
