...
resolution = "Publish the canonical scheme and canonical effect/capability contract separately from the portable typed-body payload owned by core.portable-generic-body-interface; downstream units specialize, and cross-module recursive SCCs are rejected."
...
An exported generic interface publishes its canonical scheme, canonical effect
row and capability facts, and stable dependency and ABI digests. The separate
`core.portable-generic-body-interface` child owns the versioned executable body
payload. The defining module validates both parts even without local instances.
A consuming compilation unit may specialize it under deterministic ownership.
Cross-module recursive SCCs are rejected in this slice.

## Implemented Boundary

The compiler can publish a sibling callable interface with
`--file=<module>.styio --module-id=<canonical/path>
--emit-module-interface=<module>.styioi`. Schema v4 records the canonical
...
signatures, the public contract digest, the portable typed-body aggregate
digest, the source digest, direct dependency module set and digest, compiler
ABI facts, and the resulting interface ABI digest. Payload encoding and
verification remain authoritative only in the portable-body child SSOT.
...
contract and typed-body digests, and the recomputed interface ABI before
installing any callable facts. The entry module can see only exported callables
from its direct imports; imported bodies retain access to their own private
helpers and to exported callables of their direct dependencies. Name collisions
and duplicate imports are rejected deterministically.
...
The schema v4 cutover deliberately rejects schema v3 and older metadata rather
than retaining a dual-reader compatibility path.

## Evolution Boundary

Whole-program SCCs, binary-only generic libraries, and stable public generic
ABI require separate decisions. Portable typed-body evolution belongs to
`core.portable-generic-body-interface`.
