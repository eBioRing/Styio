...
6. Semantic query cache tests: `ctest --test-dir build-codex --output-on-failure -R 'StyioSemanticDb'`
7. Stable HIR identity tests: `ctest --test-dir build-codex --output-on-failure -R 'StyioHirBuilder'`
8. Type inference query tests: `ctest --test-dir build-codex --output-on-failure -R 'StyioTypeInference|ExposesReceiverTypesForMembers|ExposesCallSiteExpectedTypes'`
9. Completion engine tests: `ctest --test-dir build-codex --output-on-failure -R 'StyioCompletionEngine'`
10. Workspace index tests: `ctest --test-dir build-codex --output-on-failure -R 'StyioWorkspaceIndex|DefinitionUsesWorkspaceIndexAcrossFiles|ReferencesMergeOpenFileAndBackgroundIndex'`
11. Runtime tests: `ctest --test-dir build-codex --output-on-failure -R 'StyioLspRuntime'`
12. Syntax drift corpus: `ctest --test-dir build-codex --output-on-failure -R 'StyioSyntaxDrift.CorpusMatchesApprovedEnvelope'`
