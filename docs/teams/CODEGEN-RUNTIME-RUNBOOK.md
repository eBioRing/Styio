...
34. Native interop diagnostic refinements are diagnostic-only. `STYIO_NATIVE_SOURCE_READ_FAILED` and `STYIO_NATIVE_SIGNATURE_NOT_FOUND` classify existing fail-closed referenced-source and explicit-binding/signature routes; do not treat those codes as new ABI support, hidden-symbol visibility, host compiler fallback, artifact linking, C++ symbol mapping, or native effect behavior.
