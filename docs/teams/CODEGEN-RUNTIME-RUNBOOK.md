...
16. Plain file acquire, write, release, line-iterator operations, acquired-handle instant pulls, and materialized list/dict/matrix index, list-slice, ordered dict value-slice, or matrix row-range slice reads outside `SIOResourceEffect` must emit the default runtime guard at the operation boundary so a materialized runtime error cannot leak into later statements. File line iteration must keep EOF separate from runtime failure, acquired-handle reads must preserve `STYIO_RUNTIME_INVALID_FILE_HANDLE` for stale handles, and list/dict/matrix bounds IR must guard the matching runtime family outside `SIOResourceEffect` after releasing owned temporaries. While lowering a `SIOResourceEffect`, suppress operation-local default guards and let the wrapper inspect `styio_runtime_has_error()` so catch-all fallback, named handlers, no-fallback settlement, and discard keep their explicit recovery semantics. For simple user-defined resource methods whose body is a single `<| expr`, accepted statement-only prefaces followed by a final `<| expr`, or scalar/local list/dict `=` / `:=` prefaces followed by a final scalar/string or local list/dict container `<| expr`, lowering must extract the returned expression before StyioIR generation so direct calls and `SIOResourceEffect` value paths produce the expression value instead of emitting an expression-context `SGReturn`; method-body inline cloning must preserve scalar leaves, statement-only prefaces, hygienically renamed scalar locals, method-local dynamic list/dict slots, `FmtStrAST`, `InstantPullAST`, `RangeAST`, `MatchCasesAST`, `FuncCallAST`, value-producing `ResourceEffectAST` result types, `ListAST`, `ListOpAST`, `DictAST`, and matrix `ListOpAST` substitutions. If an `SGBlock` with current dynamic-slot resources returns a list/dict/matrix handle as its final IR value, clone that handle and track the clone as the returned owner before block-scope cleanup; the resource-method local list/dict container return path depends on this to avoid returning a freed local dynamic slot. Dynamic-slot stores for declared `list`/`dict`/`matrix` targets must tag opaque user `SGCall` payloads by declared target type. Returned statement-only function calls, returned container match results, local matrix/resource binding method bodies, and lexical/global capture method bodies remain fail-closed, while returned resource-effect, file/stdin instant-pull, dynamic range, list/dict/matrix bounds, and local list/dict container paths keep their explicit success/fallback/handler value behavior.
