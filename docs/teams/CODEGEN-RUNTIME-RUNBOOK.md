...
21. File-resource flex rebinding must release any tracked prior file handle before overwriting the owner, then guard the cleanup boundary. Reacquiring a literal singleton path must reopen the stored slot when a prior explicit close left it at zero, while ordinary same-path aliases remain a shared singleton handle and cleanup deduplicates by slot.
