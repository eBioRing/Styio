...
31. Before JIT execution, require the current LLVM module to define its own non-declaration `main`; never satisfy an entrypoint lookup from the host process symbol table. Preserve the existing `styio: main not found` diagnostic for entrypoint-free modules.
