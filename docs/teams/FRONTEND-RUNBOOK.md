...
34. The current resource-effect parser slice accepts catch-all `?| resource_operation | fallback`, `?| resource_operation | effect => handler`, handler chains, statement-shaped resource method calls, and a final catch-all fallback after named handlers as `ResourceEffectAST`. Statement parsing keeps the value-required flag off so statement-shaped fallbacks do not become `main` return values; expression parsing sets the flag for value-producing uses such as file instant-pull fallback, untyped stdin instant-pull fallback, and explicit-target typed stdin fallback. Keep handler bodies parsed as executable resource code that stops before the next handler-chain `|`, keep `?| resource_operation | effect => ...` rejected, do not parse named handlers as ordinary fallback expressions, and leave non-resource member-call rejection to Sema instead of adding parser fallback.
