...
50. A standalone break statement at a statement boundary requires at least two contiguous carets; `^^^` is the conventional spelling and every longer contiguous run has the same single-level meaning. Reject a lone caret with the exact migration diagnostic that asks for two or more carets, and keep infix XOR plus list-caret forms outside this statement route.
