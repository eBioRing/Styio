...
50. The active stride selector is postfix `x[%n]`. Reuse the existing `%` token, construct `Access_By_Stride` with one expression slot, allow ordinary postfix composition after slices and resource snapshots, and reject a missing stride expression in both parser routes.
