...
63. Resource-effect parser/runtime changes need paired evidence: focused parser/security tests that prove non-task `?| resource_operation` forms stay out of task_await ASTs, catch-all fallback and named handlers parse through the resource-effect route, unknown or duplicate handler names fail closed, `effect => @()` remains invalid, statement-shaped file acquire is rejected where a value-producing expression is required, and non-resource member calls are rejected when method-call syntax is admitted as a resource-operation candidate. Runtime evidence must cover the smallest behavior being changed: discard still commits the underlying topology write, fallback or a matched named handler runs only after a materialized resource failure is cleared, successful operations skip recovery, unmatched named handlers without catch-all fallback keep fail-fast behavior, handler chains can fall through to a final catch-all fallback, statement-shaped file acquire proves success/fallback/named-handler/no-fallback settlement for file-open read failures, successful wrapper acquire remains usable by a following file iterator, later resource-effect close method, later guarded write method, or later acquired-handle instant pull when that topology path is accepted, close-method use consumes the receiver, recovered failed acquire followed by later iterator, write-method, or `?| (<< f) | ...` instant-pull use fails closed through the zero-handle diagnostic, guarded write-method use after a recovered failed acquire must not create the missing path, direct resource method calls such as `@file(...).close()` prove success/fallback/named-handler/no-fallback settlement, cleanup handlers catch only cleanup-family runtime subcodes, adjacent effect families do not miscatch, and no-fallback settlement stops before the following statement. Value-producing expression slices also need a positive success-value smoke, a fallback or named-handler recovery smoke, a no-fallback fail-fast smoke, and adjacent negatives for expression discard, statement-shaped operations in value context, fallback/handler type mismatch, non-file acquired-handle instant-pull sources, and nearby unsupported operation families. For value-returning resource methods, include a direct call smoke to catch expression-context `SGReturn` regressions, a `?| method() | fallback` success-value smoke, returned file instant-pull fallback/named-handler/no-fallback evidence, returned canonical stdin instant-pull fallback/`parse`-handler/no-fallback evidence, returned list index/list-slice, inline dict-index, and typed-parameter matrix cell/row or row-range slice `bounds` fallback/handler/no-fallback evidence, a returned dict-slice negative, a resource method argument type mismatch diagnostic, a lexical/global capture negative until capture semantics exist, a fallback type mismatch diagnostic, and a multi-statement return rejection before lowering until broader method-body value semantics exist. For materialized container bounds, include plain `xs[i]`, `xs[0..]`, `d[key]`, `m[row][col]`, and `m[start..end]` JSONL fail-fast tests outside `?|`, `bounds` handler recovery under `?|` for `STYIO_RUNTIME_LIST_INDEX`, `STYIO_RUNTIME_DICT_KEY`, and `STYIO_RUNTIME_MATRIX_INDEX`, matrix row and row-range recovery when those reads are accepted, list-slice fallback/no-fallback coverage when slice reads are accepted, and a dict slice boundary negative until ordered dict-slice semantics are implemented.
...
66. Topology v2 selector changes need both positive and adjacent negative evidence. When `@name[-n..]` or `@name[...]` is accepted for a resource family, add a runtime fixture that proves the selector materializes a distinct typed slice/snapshot value instead of the latest scalar, plus fail-closed fixtures for the nearest unsupported boundaries such as depth beyond the declared history window or an unsupported bounded value family. Keep the state-resource catalog current when a new family such as bounded `bool`, `char`, or `string` is closed, and move the adjacent unsupported fixture to the next real boundary such as `matrix` rather than preserving an obsolete negative.
67. Explicit selector-copy changes need the same boundary evidence: prove `name << @resource[-n..]` or `name << @resource[...]` binds the materialized snapshot value, and add an adjacent negative for scalar latest reads such as `name << @resource[-1]` so copy semantics do not collapse into ordinary scalar binding or resource-write compatibility.
...
76. Resource/state helper inline-clone slices need paired source evidence. For each newly accepted helper-body AST family, add a positive smoke that executes through the compiler binary after inlining, an adjacent fail-closed negative for the nearest invalid literal or syntax shape, and an assertion that the unsupported inlined state expression clone diagnostic is absent from the positive path. Keep examples concrete, such as `CharAST` with invalid multi-byte char rejection and `FmtStrAST` with malformed format-string rejection; do not treat either family as proof that the full helper clone surface is complete.
