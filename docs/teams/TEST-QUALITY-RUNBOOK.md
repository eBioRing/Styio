...
65. File cleanup lifecycle changes need both codegen-shape and source smoke evidence. For explicit-return cleanup, prove the LLVM IR emits `styio_file_close` before the function `ret`, run a source-level function-return smoke through the compiler binary, and rerun adjacent flex-rebind, alias-close, and cleanup-family resource-effect tests so normal cleanup, stale handles, and explicit `cleanup` handlers stay distinct.
