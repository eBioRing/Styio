...
62. Plain resource-operation settlement changes need adjacent evidence outside the explicit `?|` wrapper. For file acquire/write guards, include JSONL CLI tests that prove the runtime diagnostic is emitted and a following statement is not executed, then rerun representative `StyioResourceEffects` fallback/handler tests to prove explicit recovery still owns `?| ... | fallback` behavior.
