...
59. Callable-interface coverage must publish dependencies in a temporary tree, execute downstream scalar/string/constrained specializations including a private concrete helper edge, and round-trip schema-v2 closed/open effect rows with a transitive open-tail edge. It must independently reject private access, non-canonical module IDs, module cycles, missing interfaces, stale source/older-schema/dependency facts, and an exported body that fails without a local instance. Assert that emitted effect rows contain sorted `labels` plus nullable `open_tail` and omit legacy `bits`, `closed`, and `canonical` fields. Negative cases must assert stable diagnostic fragments and may not leave generated `.styioi` artifacts in the repository.
