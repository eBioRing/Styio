...
67. Keep direct internal test fixtures aligned with fail-closed collection coercion and iterable-write lowering: use type-valid inputs for positive codegen cases, keep mismatches in negative tests, and inspect the pulse body when a container write lowers through `SGForEach`.
