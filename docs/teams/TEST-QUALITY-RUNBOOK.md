...
59. Nano/static repository negative-path coverage is compiler-side contract evidence, not package-manager UX. Keep marker contract rejection, blob SHA256 mismatch, blob size mismatch, and remote-publish rejection covered before extending static repository behavior; add the narrowest test that asserts the stable diagnostic text for each new guard.
