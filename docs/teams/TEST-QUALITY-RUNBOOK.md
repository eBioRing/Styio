...
69. Materialized container clone coverage needs both sides of the IM-D4 copy rule: prove `name << list_or_dict_or_matrix` lowers to explicit clone IR and produces an independent runtime container after source mutation, and prove `name <- list_or_dict_or_matrix` fails closed so acquire/pull syntax does not remain a hidden clone compatibility path.
