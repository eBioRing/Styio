...
39. Native interop diagnostic refinements are public JSONL facts over existing fail-closed boundaries. Keep `STYIO_NATIVE_SOURCE_READ_FAILED` and `STYIO_NATIVE_SIGNATURE_NOT_FOUND` with phase `native_interop`, exit code 4 for the current sema/codegen TypeError route, stable message fragments, and no-following-output coverage without treating them as native ABI, symbol visibility, or toolchain behavior expansion.
