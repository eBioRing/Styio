...
32. Keep full and nano callable-interface compiler ABI identities synchronized with the active `.styioi` schema. Canonical effect-row schema v2 must use the `styio.callable-interface.v2` namespace in both binaries, reject v1 metadata before installing module facts, and bump dependent specialization/dependency fingerprint namespaces rather than retaining a dual-reader compatibility path.
