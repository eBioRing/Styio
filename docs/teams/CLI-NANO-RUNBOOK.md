...
46. Matrix literal invalid diagnostics are type-family public facts over existing fail-closed matrix literal shape/context checks. Keep `STYIO_TYPE_MATRIX_LITERAL_INVALID`, phase `type`, exit code 4 for the current TypeError route, stable matrix literal message fragments, and no-following-output coverage without treating the code as broader matrix literal acceptance, matrix return compatibility, matrix operators, resource-method matrix bodies, or matrix runtime behavior.
47. Cloud manifest nano creation must preserve the manifest-owned package identity when `--package-name` is omitted. Do not apply the local `styio-nano` fallback before manifest materialization can read `[package].name`; keep this covered by `StyioNanoPackage.CloudManifestFileUriMaterializesBundle`.
