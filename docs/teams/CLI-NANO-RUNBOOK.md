...
39. Native interop diagnostic refinements are public JSONL facts over existing fail-closed boundaries. Keep `STYIO_NATIVE_SOURCE_READ_FAILED`, `STYIO_NATIVE_SIGNATURE_NOT_FOUND`, `STYIO_NATIVE_UNSUPPORTED_SIGNATURE`, and `STYIO_NATIVE_HOST_COMPILE_FAILED` with phase `native_interop`, exit code 4 for the current sema/codegen TypeError route, stable message fragments, and no-following-output coverage without treating them as native ABI, signature support, symbol visibility, host compiler behavior, or toolchain behavior expansion.
