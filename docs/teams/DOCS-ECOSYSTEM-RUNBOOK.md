...
109. For native interop diagnostic refinements, describe the change as a public diagnostic-code refinement only: record `STYIO_NATIVE_SOURCE_READ_FAILED`, `STYIO_NATIVE_SIGNATURE_NOT_FOUND`, the missing-source and explicit-binding routes that prove them, and the unchanged TypeError exit family without implying broader native ABI support, hidden-symbol visibility, C++ symbol mapping, host compiler behavior, or native effect semantics.
