...
97. When broadening an already-closed value-producing resource-effect slice, update the gap audit, IM-D4 inventory, next-stage ledger, test catalog, owning team runbooks, and `DOC-STATS.md` with the precise new operation family and effect family. For file/stdin instant-pull closures, keep the wording limited to `i64` success values and `io`/`parse` recovery evidence unless typed stdin f64/string/list paths have their own tests.
