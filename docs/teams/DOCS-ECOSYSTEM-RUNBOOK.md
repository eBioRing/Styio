...
115. When a helper-body inline-clone slice is already implemented but lacks source-reachable evidence, update rollups and `workflows/TEST-CATALOG.md` as an evidence closure only: name the exact AST family such as scalar leaf ASTs, `FmtStrAST`, `RangeAST`, `MatchCasesAST`, or `ResourceEffectAST`, record the resource-method source path, direct-call and `?| method() | fallback` behavior, adjacent fail-closed negative, and remaining unsupported clone families; do not present the update as a new syntax or semantic expansion.
