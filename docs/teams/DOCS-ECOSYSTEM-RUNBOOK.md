...
75. Keep callable interface publication and canonical effect rows in their separate feature SSOTs. Shared views must distinguish authored `@import`/`@export` syntax from compiler-owned `.styioi` facts, identify schema v2 `labels` plus nullable `open_tail` as effect identity rather than legacy bits or stored canonical strings, state the explicit dependency-first publication order, enumerate older-schema/source/body/dependency/ABI rejection, preserve direct-export-only visibility and private body helpers, and describe whole module-cycle rejection as the current conservative recursive boundary rather than a permanent source-language theorem.
