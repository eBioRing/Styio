...
75. Keep callable interface publication in its own feature SSOT. Shared views must distinguish authored `@import`/`@export` syntax from compiler-owned `.styioi` facts, state the explicit dependency-first publication order, enumerate stale schema/source/body/dependency/ABI rejection, preserve direct-export-only visibility and private body helpers, and describe whole module-cycle rejection as the current conservative recursive boundary rather than a permanent source-language theorem.
