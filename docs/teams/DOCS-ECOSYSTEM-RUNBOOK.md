...
73. Keep higher-order callable policy and its monomorphic callable-value child in separate feature SSOTs. Shared language views must distinguish direct named scheme instantiation from contextual freezing under one complete invariant `#(...): ...` type, and limit executable values to final noncapturing named items. Do not imply rank-2 values, capturing closures, mutable callable slots, address equality, native-pointer interop, generalized storage, or callable topology from the allocation-free function-item boundary.
...
75. Keep callable interface publication and canonical effect rows in their separate feature SSOTs. Shared views must distinguish authored `@import`/`@export` syntax from compiler-owned `.styioi` facts, identify schema v3 `labels` plus nullable `open_tail` as effect identity rather than legacy bits or stored canonical strings, keep sorted per-variable usage requirements under their capability-usage SSOT, state the explicit dependency-first publication order, enumerate older-schema/source/body/dependency/ABI rejection, preserve direct-export-only visibility and private body helpers, and describe whole module-cycle rejection as the current conservative recursive boundary rather than a permanent source-language theorem.
76. Keep callable specialization policy in its own feature SSOT. Shared views must state that demand-driven mono items and their full content digests are compiler metadata, enumerate transitive callable/module/backend identity facts and both hard ceilings, scope single ownership and cache reuse to one compiler invocation, and preserve explicit instantiation, distributed caching, normal warning thresholds, and stable callable addresses as separate future decisions.
