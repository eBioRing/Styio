...
**Purpose:** Freeze the cross-repo ownership rules that let `styio-nightly`, `pafio-nightly`, and `vityo-nightly` develop in parallel without source-level coupling or undocumented API drift.
...
2. `pafio-nightly` owns package management, registry/backend services, hosted control-plane APIs, and the repo-hosted control console frontend.
3. `vityo-nightly` owns the user-facing editor/runtime frontend and consumes backend/toolchain contracts through adapter boundaries.

## Contract Ownership

1. language, compiler, and managed-toolchain truth belongs to `styio-nightly`
2. package, project graph, dependency, deployment, and hosted workspace backend contracts belong to `pafio-nightly`
3. product-facing adapter contracts and UI-facing normalized result shapes belong to `vityo-nightly`
4. cross-repo HTTP contracts ship from the owning repo as named interface packages with explicit schema markers, not as issue-thread prose or source-level conventions
...
2. `vityo-nightly` must not infer backend truth from `.spio`, compiler install layout, or other private filesystem structures when a published payload exists.
3. `styio-nightly` does not own hosted control-plane or repo-console product behavior.
4. `vityo-nightly` does not own package-manager or registry backend semantics.
