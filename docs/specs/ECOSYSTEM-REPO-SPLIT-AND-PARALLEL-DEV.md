...
**Purpose:** Freeze the cross-repo ownership rules that let `styio-nightly`, `styio-pafio`, and `styio-view` develop in parallel without source-level coupling or undocumented API drift.
...
2. `styio-pafio` owns package management, registry/backend services, hosted control-plane APIs, and the repo-hosted control console frontend.
...
2. package, project graph, dependency, deployment, and hosted workspace backend contracts belong to `styio-pafio`
...
1. `styio-pafio` must not depend on `styio-nightly` implementation headers or libraries.
2. `styio-view` must not infer backend truth from `.pafio`, compiler install layout, or other private filesystem structures when a published payload exists.
