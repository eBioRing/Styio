...
| Compiler JSONL diagnostic codes | `src/main.cpp` + `DiagnosticContract.hpp` | `styio --error-format=jsonl --file <path>` and compile-plan `diag_dir/diagnostics.jsonl` | CLI JSONL diagnostic fields | Expose stable compiler/runtime diagnostic families beyond syntax-check, including feature-owned sema/type codes such as `STYIO_SEMA_IMMUTABLE_BINDING`, `STYIO_TYPE_UNSUPPORTED_TUPLE_RETURN`, and `STYIO_TYPE_STREAM_ZIP_UNSUPPORTED_SOURCE`. |
