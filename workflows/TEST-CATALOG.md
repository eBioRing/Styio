...
| `state_resources` | `tests/features/state_resources/` | Topology-style state resources, bounded `i64`/`f64`/`bool`/`string` selector slice/snapshot and explicit selector-copy runtime evidence, unsupported selector fail-closed checks, and retired state-family negative diagnostics | `ctest --test-dir build/default -L state_resources --output-on-failure` |
| `stream_processing` | `tests/features/stream_processing/` | zip over literals, files, materialized list handles, mixed file/list pairs, and bounded selector snapshots, plus snapshots, instant pull, file-backed streams, singleton, arbitrage, stream output artifacts | `ctest --test-dir build/default -L stream_processing --output-on-failure` |
