...
| `StyioResourceEffects` file instant-pull value resource-effect tests | Prove `result = ?| (<< @file("data.txt")) | fallback` returns the successful `i64` file line or fallback value, matched `io` handlers can recover, no-fallback expression settlement fails fast, expression discard remains rejected, statement-shaped write operations are not value expressions, and fallback type mismatches fail closed. | `ctest --test-dir build/default -R '^(StyioResourceEffects\.(ValueFallbackRecoversFileInstantPull\|ValueNamedHandlerRecoversFileInstantPull\|ValueNoFallbackStopsFileInstantPull)\|StyioSecurityNightlyParserStmt\.(ParsesResourceEffectValueFallbackExpression\|RejectsResourceEffectDiscardExpression)\|StyioSecurityNightlySemantics\.(RejectsStatementShapedResourceEffectExpression\|RejectsResourceEffectValueFallbackTypeMismatch))$' --output-on-failure` |
