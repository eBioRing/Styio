...
| `StyioResourceEffects` instant-pull value resource-effect tests | Prove `result = ?| (<< @file("data.txt")) | fallback` and `result = ?| (<- @stdin) | fallback` return successful `i64` values or fallback values, matched `io`/`parse` handlers can recover file-open read and stdin numeric parse failures, no-fallback expression settlement fails fast, expression discard remains rejected, statement-shaped write operations are not value expressions, and fallback type mismatches fail closed. | `ctest --test-dir build/default -R '^(StyioResourceEffects\.(ValueFallbackRecovers(FileInstantPull\|StdinInstantPullParseFailure)\|ValueNamedHandlerRecovers(FileInstantPull\|StdinInstantPullParseFailure)\|ValueNoFallbackStops(FileInstantPull\|StdinInstantPullParseFailure))\|StyioSecurityNightlyParserStmt\.(ParsesResourceEffectValue(FallbackExpression\|StdinPullFallbackExpression)\|RejectsResourceEffectDiscardExpression)\|StyioSecurityNightlySemantics\.(RejectsStatementShapedResourceEffectExpression\|RejectsResourceEffectValueFallbackTypeMismatch))$' --output-on-failure` |
