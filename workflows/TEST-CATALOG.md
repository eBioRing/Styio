...
| `state_resources` | `tests/features/state_resources/` | Topology-style state resources, bounded `i64`/`f64`/`bool` selector slice/snapshot runtime evidence, unsupported selector fail-closed checks, and retired state-family negative diagnostics | `ctest --test-dir build/default -L state_resources --output-on-failure` |
