...
| `StyioResourceCopy` materialized clone tests | Prove `copy << list_or_dict` emits explicit clone lowering, produces independent runtime containers, and keeps `copy <- list_or_dict` fail-closed because `<-` is acquire/receive or task pull, not bound-resource clone. | `ctest --test-dir build/default -R '^(StyioSecurityNightlySemantics\.(RejectsBoundResourceCopyByLeftArrow\|AllowsExplicitCloneFormAndIndexedMutation\|AllowsDictIndexingAttrsAndClone)\|StyioResourceCopy\.ExplicitListAndDictCloneAreIndependent)$' --output-on-failure` |
