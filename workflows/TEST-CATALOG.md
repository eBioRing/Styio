...
| `state_resources` | `tests/features/state_resources/` | Topology-style state resources, bounded selector slice/snapshot runtime evidence, and retired state-family negative diagnostics | `ctest --test-dir build/default -L state_resources --output-on-failure` |
