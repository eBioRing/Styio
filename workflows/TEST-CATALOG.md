...
| `StyioDiagnostics` file resource default-settlement tests | Prove plain file acquire/write failures outside `?|` emit JSONL runtime diagnostics and stop before following statements, while explicit resource-effect recovery remains covered separately. | `ctest --test-dir build/default -R '^StyioDiagnostics\.RuntimeFile(Acquire\|Write)FailureStopsBeforeNextStatement$' --output-on-failure` |
| `StyioResourceLifecycle` flex file rebind tests | Prove use-after-close remains rejected without a rebind, accepted `name = @file(...)` creates a fresh resource occupant, and same-path singleton slots reopen after explicit close. | `ctest --test-dir build/default -R '^(StyioResourceLifecycle\.FlexFileRebindAfterCloseReopensSamePath\|StyioResourceTopology\.(ResourceUseAfterCloseStillRequiresRebind\|FlexResourceRebindClearsDestroyedReceiver))$' --output-on-failure` |
